...
// A minimal consumer `scripts` fixture. josh-game sync no longer manages package.json
...
// Framework / app-shell files that josh-game sync refreshes from templates/ on every
...
// Byte-identical, import-decoupled files single-sourced at the repo root: josh-game
...
const EXPECTED_ROOT_SYNC_FILES = josh_game_root_files.ROOT_COPY_FILES
...
// reads from, mirroring a real filesystem. Required when a single josh-game sync run
...
async function build_complete_consumer_development_deps(): Promise<Record<string, string>> {
	...
	return Object.fromEntries(
		josh_game_managed_dev_deps.REQUIRED_DEV_DEPS.map((k) => [k, overrides[k] ?? '*']),
	)
}

describe('josh_game_sync.run', () => {
	beforeEach(async () => {
	...
	it('pre-syncs pnpm-workspace.yaml BEFORE invoking pnpm josh-app sync (regression for #182)', async () => {
		...
		// josh-game sync must therefore refresh pnpm-workspace.yaml itself, BEFORE invoking pnpm.
		const { cpSync } = await import('node:fs')
		const { execSync } = await import('node:child_process')
		const { josh_game_sync } = await import('./josh-game-sync.ts')
...
describe('josh_game_sync.SYNC_FILES contract', () => {
	it('does not manage tsconfig.json or src/app.d.ts (owned by the josh-app overlay, #326/#357)', async () => {
		...
		const { josh_game_sync } = await import('./josh-game-sync.ts')
		const destinations = josh_game_sync.SYNC_FILES.map((entry) => entry.dest)
...
describe('josh_game_sync managed package.json devDependencies', () => {
	...
	it('adds missing required devDeps to consumer package.json (#186)', async () => {
		...
		// eslint, etc. josh-game sync must inject them so the next pnpm install (the
...
describe('josh_game_sync.apply_managed_dev_deps', () => {
	it('returns false and leaves pkg untouched when every required dep is present', async () => {
		...
		const did_change = josh_game_sync.apply_managed_dev_deps(package_, required)
...
describe('josh_game_sync.remove_legacy_pnpm_field', () => {
	it('removes a legacy pnpm field but leaves a package without one unchanged (#323)', async () => {
		...
		expect(josh_game_sync.remove_legacy_pnpm_field(with_field)).toBe(true)
		expect('pnpm' in with_field).toBe(false)
		expect(with_field.name).toBe('consumer')
		expect(josh_game_sync.remove_legacy_pnpm_field({})).toBe(false)
	})
})

describe('josh_game_sync.is_josh_resolvable', () => {
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it('returns true when the josh probe command succeeds', async () => {
		...
		expect(josh_game_sync.is_josh_resolvable()).toBe(true)
	...
	it('returns false when the josh probe command throws (bin unresolvable)', async () => {
		...
		expect(josh_game_sync.is_josh_resolvable()).toBe(false)
	})
})

describe('josh_game_sync.run josh-app delegation', () => {
	beforeEach(async () => {
		vi.clearAllMocks()
	...
	it('logs an actionable error and exits non-zero when josh is unresolvable', async () => {
		// Bin absent: the resolvability probe (pnpm josh help) throws. josh-game sync must
