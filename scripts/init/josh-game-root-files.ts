...
// single source of truth; package.json `files` ships each path so `josh-game init`
// / `josh-game sync` can read it from the installed package. A maintainer guard
...
// bare default lacks, and seeding only happens when the file is absent — so josh-game leaves
...
const josh_game_root_files = { ROOT_COPY_FILES, is_root_copy_file, copy_root_file }
export { josh_game_root_files }
