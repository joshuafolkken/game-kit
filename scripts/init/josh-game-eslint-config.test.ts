...
describe('josh_game_eslint_config.generate_eslint_config', () => {
	it('delegates to game-kit create_game_config, scoping happens in the preset (#368)', async () => {
		const { josh_game_eslint_config } = await import('./josh-game-eslint-config.ts')
		const result = josh_game_eslint_config.generate_eslint_config()
...
describe('josh_game_eslint_config single-sources the game-dir profile via the preset (#261, #368)', () => {
...
describe('josh_game_eslint_config.write_eslint_config', () => {
	it('writes eslint.config.js into the given project directory (#260)', async () => {
		const { writeFileSync } = await import('node:fs')
		const { josh_game_eslint_config } = await import('./josh-game-eslint-config.ts')

		josh_game_eslint_config.write_eslint_config('/project')
