...
describe('jgame_managed_scripts canonical value (integration with real package.json)', () => {
	...
	it('returns guarded prepare:* sub-scripts covering lefthook + tsx + gen without failing install (#272/#311)', async () => {
		...
		// Regression for #272/#311: each owner-only tool MUST be guarded so a missing
		// binary or config skips instead of aborting `pnpm install`, and each guarded
		// sub-script MUST end with a `true` fallback so it never propagates failure.
		...
		expect(result['prepare:lefthook'].trim()).toMatch(/;\s*true$/u)
		expect(result['prepare:gh-packages'].trim()).toMatch(/;\s*true$/u)
		expect(result['prepare:gen'].trim()).toMatch(/;\s*true$/u)
	})
})
