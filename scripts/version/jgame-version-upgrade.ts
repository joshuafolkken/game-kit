...
async function run(): Promise<void> {
	...
	const isGlobal_stale = jgame_version_check_logic.is_target_stale(global_version, latest)
	const isProject_stale = jgame_version_check_logic.is_target_stale(project_version, latest)

	if (!isGlobal_stale && !isProject_stale) {
		console.info(ALREADY_UP_TO_DATE)

		return
	}

	const status = await run_upgrades(isGlobal_stale, isProject_stale, latest)
