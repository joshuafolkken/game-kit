 ...
 *
 * Copy pairs (e.g. .npmrc → templates/npmrc) are byte-for-byte copies,
 * regenerated automatically. Tripwire pairs (e.g. vite.config.ts) intentionally
 * diverge: a source edit trips --check until the paired template is reviewed and
 * reconciled. Only the review is enforced for tripwire pairs.
 */
import { fileURLToPath } from 'node:url'
import { parseArgs } from 'node:util'
import { template_source_logic } from './template-source-logic.ts'
