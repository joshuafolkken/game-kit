...
describe('MouseDiagram', () => {
	it('renders the SVG diagram', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })

		expect(container.querySelector('svg.mouse-diagram')).toBeTruthy()
	})

	it('does not render visible action label text', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })
	...
	it('does not render visible look label text', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })
		const texts = [...container.querySelectorAll('text')]
	...
	it('does not render scroll wheel', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })
	...
	it('does not render drag arrow', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })

		expect(container.querySelector('.drag-arrow')).toBeNull()
	})

	it('uses viewBox "0 -7 90 120" so content is vertically centered', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })

		expect(container.querySelector('svg')?.getAttribute('viewBox')).toBe('0 -7 90 120')
	})

	it('all stroke-widths are <= 1 to match keyboard visual stroke width', async () => {
		const { container } = await render(MouseDiagram, { props: PROPS })
