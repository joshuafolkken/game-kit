...
describe('Room', () => {
	it('renders with explicit dimensions', async () => {
		const { container } = await render(Room, {
	...
	it('renders with default dimensions (no width/depth/height)', async () => {
		const { container } = await render(Room, { props: COLORS })
