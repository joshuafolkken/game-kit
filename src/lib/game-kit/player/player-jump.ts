...
function step_jump(jump_input: JumpInput): JumpResult {
	...
	if (new_pos_y <= jump_input.ground_y) {
		return { new_pos_y: jump_input.ground_y, new_vel_y: 0, jump_consumed: is_jump_consumed }
	}

	return { new_pos_y, new_vel_y: vel_y, jump_consumed: is_jump_consumed }
}
