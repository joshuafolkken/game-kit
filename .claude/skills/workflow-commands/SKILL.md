...
- **The verification gate**, in this order: refactor per `node_modules/@joshuafolkken/kit/prompts/refactoring.md` → **`pnpm josh gate` (lint, type check, spell check and unit tests, run concurrently) is *started* when the review starts, and *joined* before the commit** — the same treatment `josh eval` already gets below, and for the same reason: neither the gate nor the review writes to the working tree, so paying for them one after the other is pure waiting (joshuafolkken/kit#1242, measured at 187 seconds of a 1623-second run) → `/code-review` with the brief `pnpm josh review:brief` prints
  ...
  (`node_modules/@joshuafolkken/kit/prompts/review.md` → "Review round cap" and "The second round is a verification pass, not a second
  ...
  **Where a second round is coming, the pull request opens between the two** (joshuafolkken/kit#1261):
  ...
  (`node_modules/@joshuafolkken/kit/prompts/review.md` → "The round-2 fix commit is pushed before its gate"). `node_modules/@joshuafolkken/kit/prompts/review.md` →
  ...
  **A clean second round is not a turn boundary either**: the turn that reads it issues
  ...
  (`node_modules/@joshuafolkken/kit/prompts/review.md` → "A clean second round issues the merge in the same turn", the single source).
  ...
  **the unit check there is the scoped one**: `pnpm josh test:related` runs the tests the changed
  ...
  back to a single check. The table is `node_modules/@joshuafolkken/kit/prompts/review.md` → "The gate runs beside this review, not in
  front of it".
...
This section is the single source of the rule; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/human-review-label.md`
...
- **A cross-repository target is written `owner/repo#N`** and inserted from that repository's
  ...
  each checkout is (`node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/cross-repo-epic.md`). A bare `#N` resolves to
...
This section is the single source of the rule; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/into-epic.md` is a
...
This section is the single source of the rule; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/delegation.md` is a
...
This section is the single source of the rule; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/target-repo.md` is a
...
| --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| A defect originating in **another package**                                 | File the upstream Issue and **stop** — Tier A for a first-party target; a third-party one is Tier C, recorded and drafted rather than filed (`CLAUDE.md` → "Cross-package problems"; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/upstream-interrupt.md`) |
...
This section is the single source of the rule; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/prerequisite-issue.md`
...
- **The prohibition on carrying a file's new text inside a shell command** — an edit happens on any
  ...
  `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/file-edits.md`; what stays resident is the instruction and the
...
- **The instruction to put independent calls in one turn** — a tool call happens on any turn at all
  ...
  the result are at `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/turn-batching.md` (joshuafolkken/kit#1304).
