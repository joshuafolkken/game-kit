...
This file is the single source of the rule. `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/completion-notify.md` is
...
`node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/operating-rules.md`, which keeps its other operating rules and is
...
- **Waits for the CI status checks — the required ones only.** A non-required check (CodeQL, Workers
  ...
  `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/operating-rules.md` → "CI チェック失敗時の対応", which stays there
...
The portable, cross-AI wording of this section used to sit in `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/operating-rules.md` as well; it is single-sourced here now (joshuafolkken/kit#1187).
