...
This file is the single source of the `epicrun` procedure; `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/epicrun.md` is a pointer to it (joshuafolkken/kit#1188, the joshuafolkken/kit#1176 rollout of the joshuafolkken/kit#1174 pattern). That topic file used to open by declaring that the two had to agree, which is what made every rule here a rule to be written twice.
